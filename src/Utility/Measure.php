...
class Measure implements LoggerAwareInterface, ServiceLocatorAwareInterface
{
    ...
    public function init($config = [])
    {
        if (!isset($config[self::LENGTH])) {
            $config[self::LENGTH] = self::SYS_LENGTH_UNIT;
        }
        if (!isset($config[self::WEIGHT])) {
            $config[self::WEIGHT] = self::SYS_WEIGHT_UNIT;
        }
        if (!isset($config[self::VOLUME])) {
            $config[self::VOLUME] = self::SYS_VOLUME_UNIT;
        }
    ...
    public static function convertLength(
        $value,
        $toType,
        $decimalLength = self::CONVERT_ROUND,
    ...
    public static function convertVolume(
        $value,
        $toType,
        $decimalLength = self::CONVERT_ROUND,
